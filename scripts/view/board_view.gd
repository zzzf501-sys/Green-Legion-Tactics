...
func _load_textures() -> void:
	...
	for unit_type in db.units.keys():
		...
		for equip in db.equipment_for(str(unit_type)):
			...
			unit_textures[equip_key] = _load_texture("res://assets/images/%s.png" % equip_key)
	for building_type in db.buildings.keys():
		building_textures[building_type] = _load_texture(db.texture_path_for_building(str(building_type)))
	building_textures["大本营 T2"] = _load_texture("res://assets/images/大本营 T2.png")
	building_textures["大本营T3"] = _load_texture("res://assets/images/大本营T3.png")
	building_textures["战斗型T2据点"] = _load_texture("res://assets/images/战斗型T2据点.jpg")
	building_textures["资源型T2据点"] = _load_texture("res://assets/images/资源型T2据点.png")
...
func set_build_tiles(tiles: Array[Vector2i], color: Color) -> void:
	build_tiles = tiles
	build_color = color
	queue_redraw()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_WHEEL_UP and event.pressed:
		_zoom_at(to_local(get_global_mouse_position()), 1.12)
		return
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_WHEEL_DOWN and event.pressed:
		_zoom_at(to_local(get_global_mouse_position()), 1.0 / 1.12)
		return
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			...
			var clicked = drag_start.distance_to(end_pos) < 6.0
			dragging = false
			if clicked:
				var tile = screen_to_tile(end_pos)
...
func _draw_buildings() -> void:
	...
	for building in state.buildings:
		...
		if texture != null:
			...
			draw_rect(rect, Color(0.7, 0.6, 0.4), true)
		if not visible:
...
func _texture_key_for_building(building: Dictionary) -> String:
	...
	if building_type == "大本营":
		...
		if tier == 1:
			return "大本营 T2"
		return "大本营"
	if building_type == "据点" and int(building.get("outpost_tier", 0)) > 0:
...
func _draw_units() -> void:
	...
	for unit in state.units:
		...
		if texture != null:
			...
			draw_circle(rect.get_center(), _tile_size() * 0.28, PLAYER_COLORS[int(unit["pid"])])
		_draw_owner_frame(rect, int(unit["pid"]))
		if not bool(unit.get("moved", false)) and not bool(unit.get("done", false)):
...
func _draw_overlay_legend() -> void:
	var items = [
		{"label": "移动范围", "color": Color(0.18, 0.54, 1.0, 0.95), "filled": false},
		{"label": "攻击范围/目标", "color": Color(1.0, 0.12, 0.14, 0.95), "filled": false},
		{"label": "敌方威胁", "color": Color(0.72, 0.20, 1.0, 0.95), "filled": false},
		{"label": "未行动单位", "color": Color(0.95, 0.18, 0.22, 0.95), "ring": true}
	]
	var pos = Vector2(max(244.0, viewport_size.x - 520.0), 36.0)
	var width = 500.0
	var height = 28.0
	draw_rect(Rect2(pos - Vector2(8, 6), Vector2(width, height)), Color(0.04, 0.04, 0.08, 0.62), true)
	var x = pos.x
	for item in items:
		var color: Color = item["color"]
		var swatch = Rect2(Vector2(x, pos.y + 4), Vector2(16, 16))
		if bool(item.get("ring", false)):
			draw_arc(swatch.get_center(), 8.0, 0.0, TAU, 24, color, 2.0)
		else:
			draw_rect(swatch, Color(color.r, color.g, color.b, 0.16), true)
			draw_rect(swatch, color, false, 2.0)
		draw_string(ThemeDB.fallback_font, Vector2(x + 21.0, pos.y + 17.0), str(item["label"]), HORIZONTAL_ALIGNMENT_LEFT, 92.0, 13, Color(0.92, 0.92, 0.96))
		x += 96.0
